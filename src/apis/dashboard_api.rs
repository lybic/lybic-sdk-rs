...
impl<C: Connect> DashboardApiClient<C> {
    pub fn new(configuration: Arc<configuration::Configuration<C>>) -> DashboardApiClient<C> {
        DashboardApiClient { configuration }
    }
}
...
pub trait DashboardApi: Send + Sync {
    async fn get_stats(
        &self,
        org_id: &str,
    ) -> Result<crate::models::StatsResponseDto, Error<serde_json::Value>>;
}
...
impl<C: Connect + Clone + Send + Sync + 'static> DashboardApi for DashboardApiClient<C> {
    async fn get_stats(
        ...
        let mut req =
            __internal_request::Request::new(Method::GET, "/api/orgs/{orgId}/stats".to_string());
